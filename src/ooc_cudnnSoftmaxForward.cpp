...
cudnnStatus_t CUDNNWINAPI ooc_cudnnSoftmaxForward(
        ...
        if((n != y_n) || (c != y_c) || (h != y_h)){
                printf("ooc_cudnnSoftmaxForward : CUDNN_STATUS_BAD_PARAM\n");
                printf("%d, %d\n", n, y_n);
                printf("%d, %d\n", c, y_c);
                printf("%d, %d\n", h, y_h);
                printf("%d, %d\n", w, y_w);
        ...
        if((y_format != CUDNN_TENSOR_NCHW) || (x_format != CUDNN_TENSOR_NCHW)){
                printf("ooc_cudnnSoftmaxForward : CUDNN_STATUS_NOT_SUPPORTED\n");
        ...
        ooc_cudnnSoftmaxForward_optimize(n, c, h, w, x_on_device, y_on_device, y_data_size, mode, &n_step, &h_step);
        ...
        // initialize d_x
        void *d_x[3];
        size_t d_x_size = d_n * c * d_h * w * x_data_size;
        ...
        for(i_n = 0; i_n < n_step; i_n++){
                ...
                for(i_h = 0; i_h < h_step; i_h++){
                        ...
                        // update d_x
                        if(x_on_device){
                                cudnnSetTensor4dDescriptorEx(d_xDesc, x_dataType, d_n, c, d_h, w, x_nStride, x_cStride, x_hStride, x_wStride);
                                d_x[current] = (void *)x + (n_count * x_nStride + h_count * x_hStride) * x_data_size;
                        }
                        else{
                                cudnnSetTensor4dDescriptor(d_xDesc, x_format, x_dataType, d_n, c, d_h, w);
                                ooc_cudnnGetSubTensor4D(handle, xDesc, x, d_xDesc, d_x[current], n_count, 0, h_count, 0);
                        }
                        ...
                        if(y_on_device){
                                ...
                                d_y[current] = y + (n_count * y_nStride + h_count * y_hStride) * y_data_size;
                        }
                        else{
                        ...
                        // Softmax main
                        status = cudnnSoftmaxForward(handle, algo, mode, alpha, d_xDesc, d_x[current], beta, d_yDesc, d_y[current]);
                        ...
                        // update h_y
                        if(!y_on_device){
                                ooc_cudnnSetSubTensor4D(handle, yDesc, y, d_yDesc, d_y[current], n_count, 0, h_count, 0);
                        }
