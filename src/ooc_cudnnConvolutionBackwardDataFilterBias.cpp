...
cudnnStatus_t CUDNNWINAPI ooc_cudnnConvolutionBackwardDataFilterBias(
        ...
        // check parameter 
        if((n != y_n) || (w_k != y_c) || (x_c != w_c) || (b_n != 1) || (b_c != y_c) || (b_h != 1) || (b_w != 1)){
                printf("ooc_cudnnConvolutionBackwardDataFilterBias : CUDNN_STATUS_BAD_PARAM\n");
                return CUDNN_STATUS_BAD_PARAM;
        }
        if((x_format != CUDNN_TENSOR_NCHW) || (dy_format != CUDNN_TENSOR_NCHW)){
                printf("ooc_cudnnConvolutionBackwardDataFilterBias : CUDNN_STATUS_NOT_SUPPORTED\n");
        ...
        ooc_cudnnConvolutionBackwardDataFilterBias_optimize(
                            n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w,
                            x_on_device, w_on_device, dy_on_device, dx_on_device, dw_on_device, db_on_device,
        ...
        int d_y_nStride, d_y_cStride, d_y_hStride, d_y_wStride; 
                        ...
                        for(i_h = 0; i_h < h_step; i_h++){
                                ...
                                for(i_y_c = 0; i_y_c < y_c_step; i_y_c++){
                                        ...
                                        if(dy_on_device && (h_step == 1 || (i_h != 0 && i_h != h_step-1))){
                                                ...
                                                d_dy_backbias[current] = (void *)dy + (n_count * y_nStride + y_c_count * y_cStride + y_h_count_backbias * y_hStride) * dy_data_size;
                                        }
                                        else{
                                                ...
                                                d_y_nStride = d_y_c * d_y_h * y_w;
                                                d_y_cStride = d_y_h * y_w;
                                                d_y_hStride = y_w;
                                                d_y_wStride = 1;
                                                cudnnSetTensor4dDescriptorEx(d_dyDesc_backbias, dy_dataType, d_n, d_y_c, d_y_h_backbias, y_w, d_y_nStride, d_y_cStride, d_y_hStride, d_y_wStride);
                                                d_dy_backbias[current] = d_dy[current] + (y_h_count_backbias - y_h_count) * d_y_hStride * dy_data_size;
                                        }
