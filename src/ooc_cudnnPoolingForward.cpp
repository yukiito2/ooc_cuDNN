...
cudnnStatus_t CUDNNWINAPI ooc_cudnnPoolingForward(
        ...
        // check parameter 
        if((n != y_n) || (c != y_c)){
                printf("ooc_cudnnPoolingForward : CUDNN_STATUS_BAD_PARAM\n");
                printf("%d, %d\n", n, y_n);
                printf("%d, %d\n", c, y_c);
                printf("%d, %d\n", x_h, y_h);
                printf("%d, %d\n", x_w, y_w);
        ...
        if((y_format != CUDNN_TENSOR_NCHW) || (x_format != CUDNN_TENSOR_NCHW)){
                printf("ooc_cudnnPoolingForward : CUDNN_STATUS_NOT_SUPPORTED\n");
        ...
        ooc_cudnnPoolingForward_optimize(n, c, x_h, x_w, window_h, window_w, y_h, y_w, 
                            x_on_device, y_on_device, stride_h, y_data_size, &n_step, &c_step, &h_step);
        ...
        int d_x_h = (d_y_h - 1) * stride_h + window_h;
        ...
        for(i_n = 0; i_n < n_step; i_n++){
                ...
                for(i_c = 0; i_c < c_step; i_c++){
                        ...
                        for(i_h = 0; i_h < h_step; i_h++){
                                ...
                                d_x_h = (d_y_h - 1) * stride_h + window_h;
                                ...
                                if(x_on_device && (pad_h == 0 || h_step == 1 || (i_h != 0 && i_h != h_step-1))){
                                        ...
                                        d_x[current] = (void *)x + (n_count * x_nStride + c_count * x_cStride + y_h_count * x_hStride) * x_data_size;
                                }
                                else{
                                        d_x[current] = d_x_buffer[current];
                                        cudnnSetTensor4dDescriptor(d_xDesc, x_format, x_dataType, d_n, d_c, d_x_h, x_w);
                                        ooc_cudnnGetSubTensor4D(handle, xDesc, x, d_xDesc, d_x[current], n_count, c_count, y_h_count, 0);
                                }
                                ...
                                if(y_on_device){
                                        ...
                                        d_y[current] = y + (n_count * y_nStride + c_count * y_cStride + y_h_count * y_hStride) * y_data_size;
                                }
                                else{
                                ...
                                // pooling main
                                status = cudnnPoolingForward(handle, d_poolingDesc, alpha, d_xDesc, d_x[current], beta, d_yDesc, d_y[current]);
                                ...
                                // update h_y
                                if(!y_on_device){
                                        ooc_cudnnSetSubTensor4D(handle, yDesc, y, d_yDesc, d_y[current], n_count, c_count, y_h_count, 0);
                                }
