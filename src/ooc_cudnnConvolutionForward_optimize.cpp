...
double cudnnConvolutionForward_weight;
double cudnnConvolutionForward_bias;
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnConvolutionForwardProfile(){
        ...
        cudnnSetTensor4dDescriptor(yDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, n, y_c, y_h, y_w);
        ...
        for(i = 0; i < loop; i++){
                cudnnConvolutionForward(handle, &alpha, xDesc, x, wDesc, w, convDesc, CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM, workspace, 0, &beta, yDesc, y);
        }
        cudaDeviceSynchronize();
        ...
        // cudnnConvolutionForward_weight, cudnnConvolutionForward_bias
        cudnnConvolutionForward_weight = (time1 - time2) / (size1 - size2);
        cudnnConvolutionForward_bias = time1 - cudnnConvolutionForward_weight * size1;
...
double model_conv(
        ...
        size_t d_mem_y2 = d_n2 * d_y_c2 * d_y_h2 * y_w * data_size * d_y_s;
        ...
        size_t comp_conv2 = d_n2 * d_y_c2 * d_y_h2 * y_w * d_x_c2 * w_h * w_w * data_size;
        ...
        if(d_mem_x_buffer + d_mem_w + d_mem_y < d_mem_free / 3){
                ...
                double t_conv2 = cudnnConvolutionForward_weight * comp_conv2 + cudnnConvolutionForward_bias;
                double t_off = cudaMemcpy_weight * d_mem_y + cudaMemcpy_bias * d_y_s;
                double t_off2 = cudaMemcpy_weight * d_mem_y2 + cudaMemcpy_bias * d_y_s;

                double t_tmp = t_pre;
                if(t_tmp < t_conv) t_tmp = t_conv;

                double t_tmp2 = t_pre + t_conv + (x_c_step - 1) * t_tmp;
                if(t_tmp2 < t_off){
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnConvolutionForward_optimize(
        ...
        for(n_step_opt = 1; n_step_opt <= n; n_step_opt++){
                time = model_conv(n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, d_x_s, d_w_s, d_y_s, 
