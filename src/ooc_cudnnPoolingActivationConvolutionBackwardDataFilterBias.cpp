...
cudnnStatus_t CUDNNWINAPI ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias_optimize(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias(
        ...
        cudaPointerGetAttributes(&dz_attr, dz);
        ...
        bool dz_on_device = (dz_attr.memoryType == cudaMemoryTypeDevice);
        ...
        cudnnDataType_t dz_dataType;
        cudnnTensorFormat_t dz_format = CUDNN_TENSOR_NCHW;
        int dz_n, dz_c, dz_h, dz_w, dz_nStride_tmp, dz_cStride_tmp, dz_hStride_tmp, dz_wStride_tmp;
        cudnnGetTensor4dDescriptor(dzDesc, &dz_dataType, &dz_n, &dz_c, &dz_h, &dz_w, &dz_nStride_tmp, &dz_cStride_tmp, &dz_hStride_tmp, &dz_wStride_tmp);
        size_t dz_nStride = (size_t)dz_c * (size_t)dz_h * (size_t)dz_w, dz_cStride = (size_t)dz_h * (size_t)dz_w, dz_hStride = (size_t)dz_w, dz_wStride = 1;
        ...
        if((n != dy_n) || (w_k != y_c) || (x_c != w_c) || (b_n != 1) || (b_c != y_c) || (b_h != 1) || (b_w != 1)){
                printf("ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias : CUDNN_STATUS_BAD_PARAM\n");
                return CUDNN_STATUS_BAD_PARAM;
        }
        if((x_format != CUDNN_TENSOR_NCHW) || (dy_format != CUDNN_TENSOR_NCHW) 
            || (active_mode == CUDNN_ACTIVATION_CLIPPED_RELU) || (pool_stride_h != pool_stride_h) || (pool_pad_h != 0)){
                printf("ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias : CUDNN_STATUS_NOT_SUPPORTED\n");
        ...
        ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias_optimize(
                            n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, z_h, z_w,
                            x_on_device, w_on_device, y_on_device, z_on_device, dz_on_device, dy_on_device, dx_on_device, dw_on_device, db_on_device,
                            stride_h, dilation_h, pool_window_h, pool_window_w, pool_stride_h, x_data_size, &n_step, &x_c_step, &y_c_step, &h_step);
        ...
        int d_z_h = (d_y_h - pool_window_h) / pool_stride_h + 1;
        ...
        // initialize d_dz
        void *d_z[3], *d_z_buffer[3];
        size_t d_z_size = d_n * d_y_c * d_z_h * z_w * z_data_size;
        if(!z_on_device || h_step != 1){
                for(i = 0; i < num_pipeline; i++){
                        cudaMalloc((void**)&d_z[i], d_z_size);
                        d_z_buffer[i] = d_z[i];
                }
        }
        ...
        // initialize d_dz
        void *d_dz[3], *d_dz_buffer[3];
        size_t d_dz_size = d_n * d_y_c * d_z_h * z_w * z_data_size;
        if(!dz_on_device || h_step != 1){
                for(i = 0; i < num_pipeline; i++){
                        cudaMalloc((void**)&d_dz[i], d_dz_size);
                        d_dz_buffer[i] = d_dz[i];
                }
        }
        cudnnTensorDescriptor_t d_dzDesc;
        cudnnCreateTensorDescriptor(&d_dzDesc);
        ...
        cudnnSetPooling2dDescriptor(d_poolingDesc, pool_mode, maxpoolingNanOpt, pool_window_h, pool_window_w, pool_pad_h, pool_pad_w, pool_stride_h, pool_stride_w);
        ...
        for(i_n = 0; i_n < n_step; i_n++){
                ...
                for(i_x_c = 0; i_x_c < x_c_step; i_x_c++){
                        ...
                        for(i_h = 0; i_h < h_step; i_h++){
                                ...
                                d_z_h = (d_y_h - pool_window_h) / pool_stride_h + 1;

                                if(h_step == 1){ d_x_h = x_h; y_h_count = 0; d_y_h = y_h; z_h_count = 0; d_z_h = z_h; d_pad_h = pad_h; }
                                ...
                                for(i_y_c = 0; i_y_c < y_c_step; i_y_c++){
                                        ...
                                        // update d_z
                                        if(z_on_device && (h_step == 1 || (i_h != 0 && i_h != h_step-1))){
                                                cudnnSetTensor4dDescriptorEx(d_zDesc, z_dataType, d_n, d_y_c, d_z_h, z_w, z_nStride, z_cStride, z_hStride, z_wStride);
                                                d_z[current] = (void *)z + (n_count * z_nStride + y_c_count * z_cStride + z_h_count * z_hStride) * z_data_size;
                                        }
                                        else{
                                                d_z[current] = d_z_buffer[current];
                                                cudnnSetTensor4dDescriptor(d_zDesc, z_format, z_dataType, d_n, d_y_c, d_z_h, z_w);
                                                ooc_cudnnGetSubTensor4D(handle, zDesc, z, d_zDesc, d_z[current], n_count, y_c_count, z_h_count, 0);
                                        }

                                        // update d_dz
                                        if(dz_on_device && (h_step == 1 || (i_h != 0 && i_h != h_step-1))){
                                                cudnnSetTensor4dDescriptorEx(d_dzDesc, dz_dataType, d_n, d_y_c, d_z_h, z_w, dz_nStride, dz_cStride, dz_hStride, dz_wStride);
                                                d_dz[current] = (void *)dz + (n_count * dz_nStride + y_c_count * dz_cStride + z_h_count * dz_hStride) * z_data_size;
                                        }
                                        else{
                                                d_dz[current] = d_dz_buffer[current];
                                                cudnnSetTensor4dDescriptor(d_dzDesc, dz_format, z_dataType, d_n, d_y_c, d_z_h, z_w);
                                                ooc_cudnnGetSubTensor4D(handle, dzDesc, dz, d_dzDesc, d_dz[current], n_count, y_c_count, z_h_count, 0);
                                        }
                                        ...
                                        // pooling and activation bacward
                                        ...
                                        // pooling main
                                        status = cudnnPoolingBackward(handle, d_poolingDesc, &one, d_zDesc, d_z[current], d_dzDesc, d_dz[current], d_yDesc, d_y[current], &zero, d_dyDesc, d_dy[current]);

                                        // activation main
                                        status = cudnnActivationBackward(handle, activationDesc, &one, d_yDesc, d_y[current], d_dyDesc, d_dy[current], d_yDesc, d_y[current], &zero, d_dyDesc, d_dy[current]);
        ...
        cudnnDestroyTensorDescriptor(d_dzDesc);
        ...
        if(!z_on_device || h_step != 1) for(i = 0; i < num_pipeline; i++) cudaFree(d_z[i]);
        if(!dz_on_device || h_step != 1) for(i = 0; i < num_pipeline; i++) cudaFree(d_dz[i]);
