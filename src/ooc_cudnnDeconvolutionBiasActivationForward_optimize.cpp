...
double model_deconv_addbias_activ(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
        ...
        size_t comp_deconv = d_n * d_x_c * d_x_h * x_w * d_y_c * w_h * w_w * data_size;
        ...
        if(d_mem_x_buffer + d_mem_w + d_mem_y < d_mem_free / 3){
                ...
                double t_deconv = cudnnConvolutionBackwardData_weight * comp_deconv + cudnnConvolutionBackwardData_bias;
                ...
                double t_tmp1 = t_pre;
                if(t_tmp1 < t_deconv) t_tmp1 = t_deconv;

                double t_tmp2 = t_pre + t_deconv + (x_c_step - 1) * t_tmp1;
                if(t_tmp2 < t_off){
                        t = t_tmp2 + n_step * y_c_step * h_step * t_off;
                }
                else if(t_pre < t_deconv){
                        t = t_pre + cudnnConvolutionBackwardData_weight * n * x_c * x_h * x_w * y_c * w_h * w_w * data_size
                            + n_step * y_c_step * x_c_step * h_step * cudnnConvolutionBackwardData_bias + t_off;
                }
                else{
                        t = n_step * y_c_step * x_c_step * h_step * t_pre + t_deconv + t_off;
                }
        }
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnDeconvolutionBiasActivationForward_optimize(
        ...
        for(n_step_opt = 1; n_step_opt <= n; n_step_opt++){
                time = model_deconv_addbias_activ(n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, d_x_s, d_w_s, d_b_s, d_y_s, 
