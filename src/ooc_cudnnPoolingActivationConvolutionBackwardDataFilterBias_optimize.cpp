...
extern double cudnnPoolingBackward_weight;
extern double cudnnPoolingBackward_bias;
extern double cudnnActivationBackward_weight;
extern double cudnnActivationBackward_bias;
...
double model_pool_activ_convbackdatafilterbias(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
        ...
        size_t d_z_h2 = (d_y_h2 - pool_window_h) / pool_stride_h + 1;
        ...
        size_t d_y_h = (d_x_h + dilation_w_h - 1) / stride_h;
        size_t d_z_h = (d_y_h - pool_window_h) / pool_stride_h + 1;
        if(d_y_h < 1) return t;
        

        
        //check parameter
        size_t max_size = (size_t)(2.0 * 1024.0 * 1024.0 * 1024.0);
        ...
        size_t d_mem_z = d_n * d_y_c * d_z_h * z_w * data_size * d_z_s;
        size_t d_mem_dz = d_n * d_y_c * d_z_h * z_w * data_size * d_dz_s;
        ...
        size_t d_mem_z_buffer = d_mem_z;
        size_t d_mem_dz_buffer = d_mem_dz;
        if(h_step != 1){
                if(d_y_h2 % pool_stride_h != 0) return t;
                if(d_y_h % pool_stride_h != 0) return t;
                d_mem_y_buffer = d_n * d_y_c * d_y_h * y_w * data_size;
                d_mem_dy_buffer = d_n * d_y_c * d_y_h * y_w * data_size;
                d_mem_z_buffer = d_n * d_y_c * d_z_h * z_w * data_size;
                d_mem_dz_buffer = d_n * d_y_c * d_z_h * z_w * data_size;
        }

        if(d_y_c % 32 != 0) d_y_c = 32 * (d_y_c / 32 + 1);
        size_t comp_conv1 = d_n * d_x_c * (d_x_h / stride_h) * y_w * d_y_c * w_h * w_w * data_size;
        size_t comp_activ1 = d_n * d_y_c * d_y_h * y_w * data_size;
        size_t comp_pool1 = d_n * d_y_c * d_z_h * z_w * pool_window_h * pool_window_w * data_size;
        if(d_y_c2 % 32 != 0) d_y_c2 = 32 * (d_y_c2 / 32 + 1);
        size_t comp_conv2 = d_n2 * d_x_c2 * (d_x_h2 / stride_h) * y_w * d_y_c2 * w_h * w_w * data_size;
        size_t comp_activ2 = d_n2 * d_y_c2 * d_y_h2 * y_w * data_size;
        size_t comp_pool2 = d_n2 * d_y_c2 * d_z_h2 * z_w * pool_window_h * pool_window_w * data_size;


        if(d_mem_x + d_mem_w + d_mem_y_buffer + d_mem_z_buffer + d_mem_dz_buffer + d_mem_dy_buffer + d_mem_dx + d_mem_dw + d_mem_db < d_mem_free / 3){
                double t_pre1 = cudaMemcpy_weight * d_mem_x + cudaMemcpy_bias * d_x_s;
                double t_pre2 = cudaMemcpy_weight * (d_mem_w + d_mem_y + d_mem_z + d_mem_dz + d_mem_dw + d_mem_db)
                              + cudaMemcpy_bias * (d_w_s + d_y_s + d_z_s + d_dz_s + d_dw_s + d_db_s);
                double t_conv = (cudnnConvolutionBackwardData_weight + cudnnConvolutionBackwardFilter_weight) * comp_conv1 + cudnnConvolutionBackwardData_bias + cudnnConvolutionBackwardFilter_bias
                              + (cudnnConvolutionBackwardBias_weight + cudnnActivationBackward_weight) * comp_activ1 + cudnnConvolutionBackwardBias_bias + cudnnActivationBackward_bias
                              + cudnnPoolingBackward_weight * comp_pool1 + cudnnPoolingBackward_bias;
                double t_conv2 = (cudnnConvolutionBackwardData_weight + cudnnConvolutionBackwardFilter_weight) * comp_conv2 + cudnnConvolutionBackwardData_bias + cudnnConvolutionBackwardFilter_bias
                              + (cudnnConvolutionBackwardBias_weight + cudnnActivationBackward_weight) * comp_activ2 + cudnnConvolutionBackwardBias_bias + cudnnActivationBackward_bias
                              + cudnnPoolingBackward_weight * comp_pool2 + cudnnPoolingBackward_bias;
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnPoolingActivationConvolutionBackwardDataFilterBias_optimize(
        ...
        size_t d_z_s = 0; if(!z_on_device) d_z_s = 1;
        size_t d_dz_s = 0; if(!dz_on_device) d_dz_s = 1;
        ...
        for(n_step_opt = 1; n_step_opt <= n; n_step_opt++){
                time = model_pool_activ_convbackdatafilterbias(n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, z_h, z_w, 
                                   d_x_s, d_w_s, d_y_s, d_z_s, d_dz_s, d_dy_s, d_dx_s, d_dw_s, d_db_s, 
                                   stride_h, dilation_h, pool_window_h, pool_window_w, pool_stride_h, data_size, d_mem_free, n_step_opt, x_c_step_opt, y_c_step_opt, h_step_opt);
                if(time < time_opt){
                        time_opt = time;
                        tmp = n_step_opt;
                }
