...
double model_activ_convbackdatafilterbias(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
        ...
        if(d_mem_x + d_mem_w + d_mem_y_buffer + d_mem_dy_buffer + d_mem_dx + d_mem_dw + d_mem_db < d_mem_free / 3){
                double t_pre1 = cudaMemcpy_weight * d_mem_x + cudaMemcpy_bias * d_x_s;
                double t_pre2 = cudaMemcpy_weight * (d_mem_w + d_mem_y + d_mem_dy + d_mem_dw + d_mem_db) + cudaMemcpy_bias * (d_w_s + d_y_s + d_dy_s + d_dw_s + d_db_s);
                double t_conv = (cudnnConvolutionBackwardData_weight + cudnnConvolutionBackwardFilter_weight) * comp_conv1 + (cudnnConvolutionBackwardBias_weight + cudnnActivationBackward_weight) * comp_activ1
                              + cudnnConvolutionBackwardData_bias + cudnnConvolutionBackwardFilter_bias + cudnnConvolutionBackwardBias_bias + cudnnActivationBackward_bias;
                double t_conv2 = (cudnnConvolutionBackwardData_weight + cudnnConvolutionBackwardFilter_weight) * comp_conv2 + (cudnnConvolutionBackwardBias_weight + cudnnActivationBackward_weight) * comp_activ2
                              + cudnnConvolutionBackwardData_bias + cudnnConvolutionBackwardFilter_bias + cudnnConvolutionBackwardBias_bias + cudnnActivationBackward_bias;
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnActivationConvolutionBackwardDataFilterBias_optimize(
        ...
        for(n_step_opt = 1; n_step_opt <= n; n_step_opt++){
                time = model_activ_convbackdatafilterbias(n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, 
                                   d_x_s, d_w_s, d_y_s, d_dy_s, d_dx_s, d_dw_s, d_db_s, 
