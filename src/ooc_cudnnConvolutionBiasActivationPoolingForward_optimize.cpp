...
extern double cudnnPoolingForward_weight;
extern double cudnnPoolingForward_bias;
extern double cudaMemcpy_weight;
extern double cudaMemcpy_bias;


double model_conv_addbias_activ_pool(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
        ...
        size_t comp_pool = d_n * d_y_c * d_z_h * z_w * pool_window_h * pool_window_w * data_size;

        if(d_mem_x_buffer + d_mem_w + d_mem_y + d_mem_z < d_mem_free / 3){
                ...
                double t_off =  cudaMemcpy_weight * d_mem_b + cudaMemcpy_bias * d_b_s;
                                ...
                                + cudnnPoolingForward_weight * comp_pool + cudnnPoolingForward_bias
                                + cudaMemcpy_weight * d_mem_z + cudaMemcpy_bias * d_z_s;
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnConvolutionBiasActivationPoolingForward_optimize(
        ...
        for(n_step_opt = 1; n_step_opt <= n; n_step_opt++){
                time = model_conv_addbias_activ_pool(n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, z_h, z_w, d_x_s, d_w_s, d_b_s, d_y_s, d_z_s, 
                                   stride_h, dilation_h, pool_window_h, pool_window_w, pool_stride_h, data_size, d_mem_free, n_step_opt, y_c_step_opt, x_c_step_opt, h_step_opt);
                if(time < time_opt){
                        time_opt = time;
                        tmp = n_step_opt;
                }
