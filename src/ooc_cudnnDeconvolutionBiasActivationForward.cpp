...
cudnnStatus_t CUDNNWINAPI ooc_cudnnDeconvolutionBiasActivationForward_optimize(
                                const size_t                        n,
                                const size_t                        x_c,
                                const size_t                        x_h,
                                const size_t                        x_w,
...
cudnnStatus_t CUDNNWINAPI ooc_cudnnDeconvolutionBiasActivationForward(
        ...
        if((n != y_n) || (x_c != w_c) || (w_k != y_c) || (bias_n != 1) || (bias_c != y_c) || (bias_h != 1) || (bias_w != 1)){
                printf("ooc_cudnnConvolutionBiasActivationForward : CUDNN_STATUS_BAD_PARAM\n");
                printf("%d, %d\n", n, y_n);
                printf("%d, %d\n", x_c, w_c);
                printf("%d, %d\n", w_k, y_c);
        ...
        if((x_format != CUDNN_TENSOR_NCHW) || (y_format != CUDNN_TENSOR_NCHW)){
                printf("ooc_cudnnConvolutionBiasActivationForward : CUDNN_STATUS_NOT_SUPPORTED\n");
        ...
        ooc_cudnnDeconvolutionBiasActivationForward_optimize(
                            n, x_c, x_h, x_w, w_h, w_w, y_c, y_h, y_w, x_on_device, w_on_device, bias_on_device, y_on_device,
        ...
        int d_y_h = y_h / h_step + stride_h; if(0 < y_h % h_step) d_y_h++;
        int d_x_h = d_y_h - 1 + dilation_w_h;
        ...
        // initialize d_w
        void *d_w[3];
        size_t d_w_size = d_x_c * d_y_c * w_h * w_w * w_data_size;

        if((!w_on_device) || (d_y_c != y_c)){
        ...
        // convolution loop
        int i_n, i_y_c, i_x_c, i_h;

        int n_count, y_c_count, x_c_count, y_h_count, x_h_count;

        int pad_h_top = pad_h;
        if(y_h != (x_h - 1) * stride_h + dilation_w_h - 2 * pad_h) pad_h_top--;
        ...
        for(i_n = 0; i_n < n_step; i_n++){
                ...
                for(i_y_c = 0; i_y_c < y_c_step; i_y_c++){
                        ...
                        for(i_h = 0; i_h < h_step; i_h++){
                                ...
                                tmp2 = ((y_h - y_h_count - tmp1) / stride_h) / (h_step - i_h);
                                if(tmp2 <= 0) tmp2 = 1;
                                d_y_h = tmp2 * stride_h + tmp1;


                                d_x_h = (d_y_h + (2 * d_pad_h - dilation_w_h)) / stride_h + 1;

                                if(h_step == 1){ d_y_h = y_h; d_x_h = x_h; d_pad_h = pad_h; }
                                ...
                                for(i_x_c = 0; i_x_c < x_c_step; i_x_c++){
                                        ...
                                        // update d_w
                                        cudnnSetFilter4dDescriptor(d_wDesc, w_dataType, w_format, d_x_c, d_y_c, w_h, w_w);
                                        if(w_on_device && (d_y_c == y_c)){
                                                d_w[current] = (void *)w + x_c_count * y_c * w_h * w_w * w_data_size; 
                                        }
                                        else{
                                                ooc_cudnnGetSubFilter4D(handle, wDesc, w, d_wDesc, d_w[current], x_c_count, y_c_count, 0, 0);
                                        }
                                        ...
                                        // convolution main
                                        if(i_x_c == 0){
                                                status = cudnnConvolutionBackwardData(handle, &one, d_wDesc, d_w[current], d_xDesc, d_x[current], 
        ...
        for(i = 0; i < 3; i++){
                cudaStreamSynchronize(streams_D2H[i]);
                cudaStreamDestroy(streams_D2H[i]);
        }
